...
test('appends default parameters to URL without existing parameters', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {apiKey: 'abc', format: 'json'});
...
test('merges with existing URL parameters', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {apiKey: 'abc'});
...
test('per-call URL parameters override defaults', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {page: '1', apiKey: 'abc'});
...
test('multi-value URL parameters fully override default for that key', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {tags: 'default'});
	const response = await fetchWithParameters('/users?tags=a&tags=b');
...
test('passes options through for URL objects', async t => {
	...
	const fetchWithParameters = withSearchParameters(capturingFetch, {apiKey: 'abc'});
...
test('accepts URLSearchParams as defaults', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, new URLSearchParams({apiKey: 'abc'}));
	const response = await fetchWithParameters('/users');
...
test('URL-encodes parameter values', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {query: 'hello world'});
...
test('composes with withBaseUrl in pipeline', async t => {
	...
	const apiFetch = pipeline(
		...
		f => withSearchParameters(f, {apiKey: 'abc'}),
	);
...
test('multi-value defaults via tuples', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, [['tags', 'a'], ['tags', 'b']]);
	const response = await fetchWithParameters('/users');
...
test('stacking two withSearchParameters wrappers merges both', async t => {
	const inner = withSearchParameters(mockFetch, {apiKey: 'abc'});
	const outer = withSearchParameters(inner, {format: 'json'});
...
test('per-call parameters override both layers of stacked wrappers', async t => {
	const inner = withSearchParameters(mockFetch, {apiKey: 'abc', format: 'xml'});
	const outer = withSearchParameters(inner, {format: 'json'});
...
test('special characters in parameter values are encoded', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {redirect: 'https://example.com/cb?ok=1'});
...
test('special characters in parameter names are encoded', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {'my key': 'value'});
	const response = await fetchWithParameters('/search');
...
test('composes with withBaseUrl without pipeline helper', async t => {
	...
	const fetchWithBaseUrl = withBaseUrl(capturingFetch, 'https://api.example.com');
	const fetchWithBoth = withSearchParameters(fetchWithBaseUrl, {apiKey: 'abc'});
...
test('normalizes resolved URL objects before merging search parameters', async t => {
	...
	const fetchWithParameters = withSearchParameters(metadataAwareFetch, {
		apiKey: 'abc',
	});
	const response = await fetchWithParameters('/users');
...
test('all default parameters overridden leaves only per-call parameters', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {a: '1', b: '2'});
...
test('empty default parameter value is preserved', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {flag: ''});
	const response = await fetchWithParameters('/users');
...
test('preserves timeout metadata through copyFetchMetadata', async t => {
	...
	const fetchWithParameters = withSearchParameters(timedFetch, {apiKey: 'abc'});
...
test('does not mutate defaults between calls', async t => {
	...
	const fetchWithParameters = withSearchParameters(mockFetch, defaults);
...
test('unicode parameter values', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, {name: 'caf\u00E9'});
	const response = await fetchWithParameters('/search');
...
test('per-call parameter with same key but multiple values replaces all default values for that key', async t => {
	const fetchWithParameters = withSearchParameters(mockFetch, [['color', 'red'], ['color', 'blue']]);
